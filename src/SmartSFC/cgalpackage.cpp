...
bool OutsidePointInsideTriangle(Poly_facet_iterator &face, Vector3d p)
{
	...
	if (true)
	{
		...
		if (bbbb1 != bbbb0)
		{
			...
			if (DebugInformation())
			{
				std::ofstream  export_file_output("Z:\\debugdebug.obj");
...
void ComputeRemeshTriangles(const Vector3d1 &vecs,const std::vector<int> &face_id_0, const std::vector<int> &face_id_1, const std::vector<int> &face_id_2,
	const std::vector<std::vector<int>> &edges, const Vector3d1 &cutting_points,const std::vector<int> &multi_cps, const std::vector<int> &two_class_lables, const std::string output_path)
{
	auto HandleBoundaryTriangle=[](const Vector3d1 &vecs, const std::vector<int> &remesh_lables, const std::vector<std::vector<int>> &edges, const Vector3d1 &cutting_points,
	...
	{
		auto GetEdgeMiddlePoint = [](const int index_0, const int index_1, const  Vector3d v_0, const Vector3d v_1, const std::vector<std::vector<int>> &edges, const Vector3d1 &cutting_points)
		{
			for (int i = 0; i < edges.size(); i++)
				if ((edges[i][0] == index_0&&edges[i][1] == index_1) || (edges[i][1] == index_0&&edges[i][0] == index_1))
					return i;
		...
		int cut_0_1 = GetEdgeMiddlePoint(index_0, index_1, v_0, v_1, edges, cutting_points);
		int cut_0_2 = GetEdgeMiddlePoint(index_0, index_2, v_0, v_2, edges, cutting_points);
	...
	if (true)
	{
		...
		{
			std::ofstream  export_file_output("Z:\\cutting_point_ids_"+std::to_string(i)+".obj");
			int export_index = 1;
		...
		std::vector<std::vector<int>> &remesh_triangles, std::vector<int> &remesh_triangles_lables)
	{
		...
		{
			remesh_triangles.emplace_back(std::vector<int>{nids[triangle[0]], nids[triangle[1]], nids[triangle[2]]});
...
void ComputeEdgeLables(Polyhedron_3 &polyhedron, Halfedge_handle &start_hh, std::vector<std::vector<int>> &edges, std::vector<int> &lables)
{
	std::vector<int> two_class_lables(polyhedron.size_of_vertices(), 0);
	std::vector<bool> edge_lables(polyhedron.size_of_vertices(), false);
	std::vector<bool> over(polyhedron.size_of_vertices(), false);
	for (int i = 0; i < edges.size(); i++)
	{
		edge_lables[edges[i][0]] = true;
		edge_lables[edges[i][1]] = true;
	}
	...
	while (queue.size() != 0)
	{
		...
		if (iter%100==0)
		std::cerr << iter << ": " << hh->vertex()->id() << " / " << polyhedron.size_of_vertices() << std::endl;
		//////////
		...
		if (false)
		{
			std::ofstream  export_file_output_0("Z:\\cutting\\" + std::to_string(iter) + "_" + std::to_string(hh->vertex()->id()) + "_" + std::to_string(queue_1.front()) + ".obj");
			int export_index_0 = 1;
		...
		do
		{
			...
			if (two_class_lables[opposite_id] == 0 && !over[opposite_id])
			{
				...
				else
				{
					...
					for (int i = 0; i < edges.size(); i++)
					{
						if ((edges[i][0] == id&&edges[i][1] == opposite_id) || (edges[i][1] == id&&edges[i][0] == opposite_id))
						{
							b = true;
							break;
						}
	...
	{
		std::ofstream  export_file_output_0("Z:\\debug.obj");
		int export_index_0 = 1;
